...
public partial struct LatestTestEvent
{
    public int Value;
}
...
public partial struct MaxPendingTestEvent
{
    public int Value;
}
...
{
    public override EventPostPolicy? PostPolicy =>
        new EventPostPolicy(PostDeliveryMode.Normal, BackpressurePolicy.RejectNew, 2);
}

public partial struct DropOldestTestEvent
{
    public int Value;
}
...
public class PostSchedulerTests
{
    ...
    public void Backpressure_DropOldest()
    {
        ...
        scheduler.BuildPlans(new[]
        {
            new PostTypePlan(EventTypeId<DropOldestTestEvent>.Id, PostDeliveryMode.Normal, options.DefaultBackpressure,
                0, options.DefaultBackpressure)
        });
    ...
    public void AddSpecialPolicy_Ensures_Capacity()
    {
        ...
        scheduler.AddSpecialPolicy(typeId,
            new EventPostPolicy(PostDeliveryMode.Latest, BackpressurePolicy.RejectNew, 0));
