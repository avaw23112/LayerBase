...
public class LifecycleFixTests
{
    ...
    public void Singleton_Cross_Runtime_Conflict_Throws()
    {
        ...
        var layer2 = new TestLayer(services => 
        {
    ...
    public void EventStore_Dispose_Clears_Buffers()
    {
        ...
        var bufferField = typeof(EventStore<TestEvent>).GetField("_buffer", BindingFlags.NonPublic | BindingFlags.Instance);
    ...
    public void DI_Throws_On_Ambiguous_Public_Constructors()
    {
        var layer = new TestLayer(services => 
        {
            services.AddTransient<AmbiguousCtorService, AmbiguousCtorService>();
        });
        var builder = LayerHub.CreateLayers().Push(layer);
    ...
    {
        public int UsedCtor;
        public MultiCtorService() { UsedCtor = 0; }
        public MultiCtorService(GlobalSingleton g) { UsedCtor = 1; }
        private MultiCtorService(GlobalSingleton g, int x) { UsedCtor = 2; }
    }

    private class MountCtorService
    {
        public int UsedCtor;
        public MountCtorService() { UsedCtor = 1; }

        [Mount]
        private MountCtorService(GlobalSingleton g) { UsedCtor = 2; }
    }
