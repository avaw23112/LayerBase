...
public sealed class PooledChunkedOverwriteQueue<T> : IDisposable where T : struct
{
    ...
    internal void Prewarm(int capacity)
    {
        ...
        var neededSegments = (capacity + _chunkSize - 1) / _chunkSize;
        var existingSegments = CountSegments();
        ...
        var toAdd = neededSegments - existingSegments;
        for (var i = 0; i < toAdd; i++) AppendNewTailSegment();
    }
    ...
    private void EnqueueFront(in T item, EventQueueOverflowStrategy strategy)
    {
        ...
        if (_headIndex == 0) PrependNewHeadSegment();
    ...
    {
        if (_maxCapacity <= 0 || Count < _maxCapacity)
            return true;
    ...
    private void ScaleCapacity()
    {
        ...
        var desired = Count + 1;
        var growth = Math.Max(_chunkSize, _maxCapacity);
        var next = (long)_maxCapacity + growth;
        if (next < desired)
            next = desired;
    ...
    private void EnsureTailSegmentHasSpace()
    {
        ...
        if (_tailIndex >= _chunkSize) AppendNewTailSegment();
    }
    ...
    private void AppendNewTailSegment()
    {
        ...
        if (_tailSeg != null) _tailSeg.Next = newSeg;
    ...
    {
        if (_tailSeg != null && _tailIndex == 0 && _tailSeg.Prev != null) MoveTailToPreviousSegment();
    }
    ...
    private void DropOldest(int n)
    {
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            if (_headIndex >= _chunkSize) AdvanceHeadSegment();
        }
    }
