...
namespace LayerBase.Layers
{
    public abstract class Layer : Node, ILayerContext
    {
        ...
        public virtual void Pump(float deltaTime)
        {
            ...
            for (int i = 0; i < m_serviceUpdates.Count; i++) m_serviceUpdates[i].Update();
        }

        public void Subscribe<T>(EventHandleDelegate<T> handler) where T : struct
        {
            if (RouteIndex != -1) LayerHub.LayerHub.EventCenter.Subscribe<T>(RouteIndex, handler);
        ...
        {
            if (RouteIndex != -1) LayerHub.LayerHub.EventCenter.SubscribeAsync<T>(RouteIndex, handler);
        ...
        {
            if (RouteIndex != -1) 
                LayerHub.LayerHub.EventCenter.SubscribeParallel<T>(RouteIndex, handler, reportError ?? LayerHub.LayerHub.ReportLayerEventError);
            else 
        ...
        public EventHandledState SendLocal<T>(in T value) where T : struct => LayerHub.LayerHub.EventCenter.SendLocal(RouteIndex, value);
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EventHandledState SendBubble<T>(in T value) where T : struct => LayerHub.LayerHub.EventCenter.Send(value, RouteIndex, Propagation.Bubble);
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EventHandledState SendDrop<T>(in T value) where T : struct => LayerHub.LayerHub.EventCenter.Send(value, RouteIndex, Propagation.Drop);
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public EventHandledState SendGlobal<T>(in T value) where T : struct => LayerHub.LayerHub.EventCenter.Send(value, RouteIndex, Propagation.Global);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void PostLocal<T>(in T value) where T : struct => LayerHub.LayerHub.EventCenter.PostLocal(RouteIndex, value);
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void PostBubble<T>(in T value) where T : struct => LayerHub.LayerHub.EventCenter.Post(value, RouteIndex, Propagation.Bubble);
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void PostDrop<T>(in T value) where T : struct => LayerHub.LayerHub.EventCenter.Post(value, RouteIndex, Propagation.Drop);
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void PostGlobal<T>(in T value) where T : struct => LayerHub.LayerHub.EventCenter.Post(value, RouteIndex, Propagation.Global);
    }
}
