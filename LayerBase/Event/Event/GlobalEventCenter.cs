...
public sealed class GlobalEventCenter
{
    ...
    {
        public int SyncStart, SyncCount, AsyncStart, AsyncCount, ParallelStart, ParallelCount;
    }
    ...
    private sealed class EventBucket<T> : IResetable where T : struct
    {
        ...
        private EventHandledState DispatchSync(int start, int end, in T value)
        {
            ...
            try
            {
                // 4x 循环展开：步进式更新索引 i 以确保异常发生时 i 精确指向故障点
                for (; i <= end - 4; )
                {
            ...
            catch (Exception e)
            {
                ...
                i++; // 🚩 故障隔离：跳过故障点
                if (i < end) goto RESUME; // 🚀 原地复活，继续执行后续 Handler
            }
