...
internal sealed class DelayPublisher<T> : IDelayPublisher<T>, IDelayPublisherInternal where T : struct
{
    ...
    private readonly Action _markDelayDirty;
    ...
    public DelayPublisher(DelayPublisherManager manager, Layer owner)
        : this(
            ...
            owner.OwnerContext == null
                ? (Action)(() => { })
                : owner.OwnerContext.MarkDelayDirty)
    {
    }

    internal DelayPublisher(DelayPublisherManager manager, Action markDelayDirty)
        : this(manager, null, markDelayDirty)
    {
    }

    private DelayPublisher(DelayPublisherManager manager, Layer? owner, Action markDelayDirty)
    {
        _manager = manager;
        _markDelayDirty = markDelayDirty ?? throw new ArgumentNullException(nameof(markDelayDirty));
        Owner = owner;
    }
