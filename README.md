...
public struct BattleTick
{
    ...
    public BattleTick(float deltaTime)
    {
        DeltaTime = deltaTime;
    }
}
...
public sealed partial class BattleContext : ILayerContext
{
}
...
public sealed partial class BattleService : IService, IUpdate
{
    [Mount] private BattleContext _context = default!;
    ...
    {
        _context.Query<Position, Velocity>()
            .ForEach((ref Position position, in Velocity velocity) =>
            {
                position.Value += velocity.Value * deltaTime;
            });

        MonsterActor monster = _context.CreateActor<MonsterActor>();
        ActorId monsterId = monster.GetActorId();

        _context.PostActor(monsterId, new BattleTick(deltaTime));
        _context.Post(new BattleTick(deltaTime));

        InventoryService inventory = _context.Get<InventoryService>();
    }
}
```
...
- `context.Post(...)`
- `context.Query<...>()`
- `context.CreateActor<TActor>()`
- `context.CreatePooledActor<TActor>()`
- `context.PostActor(actorId, ...)`
- `context.Ask<TRequest, TResponse>(actorId, ...)`
- `context.DestroyActor(actorId)`
- `context.Get<TService>()`

`context.Actors()` 与 `context.ECSWorld()` 仍然保留，但应视为 Advanced API。普通业务代码优先不要直接依赖它们。

更完整的 simple guide 见 [docs/api/simple/context-first.md](docs/api/simple/context-first.md)。
...
## 🧭 Context-First Simple Entry

For day-to-day business code, prefer writing against `ILayerContext`.
...
`context.Actors()` and `context.ECSWorld()` are still supported, but they should be treated as advanced APIs rather than the default business-code entry points.

For the fuller simple guide, see [docs/api/simple/context-first.md](docs/api/simple/context-first.md).
