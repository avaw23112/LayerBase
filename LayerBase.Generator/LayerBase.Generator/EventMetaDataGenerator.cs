...
public sealed class EventMetaDataGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(compilationAndRegistrations, static (spc, source) =>
        {
            ...
            if (eventMetaDataBase == null || registryType == null) return;
            ...
            {
                foreach (var diagnostic in registration.Diagnostics) spc.ReportDiagnostic(diagnostic);

                if (!registration.IsValid) continue;

                if (registration.EventType is not INamedTypeSymbol eventTypeSymbol) continue;

                if (!IsPartial(eventTypeSymbol))
                {
                    spc.ReportDiagnostic(Diagnostic.Create(Diagnostics.EventTypeMustBePartial,
                        registration.MetaDataType.Locations.FirstOrDefault(), eventTypeSymbol.ToDisplayString()));
                    continue;
                }

                if (!registeredEvents.Add(eventTypeSymbol))
                {
                    spc.ReportDiagnostic(Diagnostic.Create(Diagnostics.DuplicateMetaDataForEventType,
                        registration.MetaDataType.Locations.FirstOrDefault(), eventTypeSymbol.ToDisplayString()));
                    continue;
                }
    ...
    private static MetaDataRegistration? CreateRegistration(GeneratorSyntaxContext context,
                                                            CancellationToken      cancellationToken)
    {
        var classDeclaration = (ClassDeclarationSyntax)context.Node;
        var typeSymbol = context.SemanticModel.GetDeclaredSymbol(classDeclaration, cancellationToken);
        if (typeSymbol == null) return null;

        var eventMetaDataSymbol = context.SemanticModel.Compilation.GetTypeByMetadataName(EventMetaDataBaseName);
        if (eventMetaDataSymbol == null) return null;
        ...
        if (typeSymbol.IsAbstract)
            diagnostics.Add(Diagnostic.Create(Diagnostics.MetaDataCannotBeAbstract, location,
                typeSymbol.ToDisplayString()));

        if (typeSymbol.TypeParameters.Length > 0)
            diagnostics.Add(Diagnostic.Create(Diagnostics.MetaDataCannotBeGeneric, location,
                typeSymbol.ToDisplayString()));

        if (!HasAccessibleParameterlessConstructor(typeSymbol))
            diagnostics.Add(Diagnostic.Create(Diagnostics.MetaDataNeedsPublicParameterlessConstructor, location,
                typeSymbol.ToDisplayString()));
    ...
    private static string GenerateSource(IEnumerable<MetaDataRegistration> registrations, string assemblyName)
    {
        ...
        if (grouped.Count == 0) return string.Empty;
        ...
        {
            if (group.Key is not INamedTypeSymbol eventType) continue;
    ...
    private static string Sanitize(string value)
    {
        ...
        foreach (var ch in value) sanitized.Append(char.IsLetterOrDigit(ch) ? ch : '_');

        if (sanitized.Length == 0) sanitized.Append("Assembly");
    ...
    private static class Diagnostics
    {
        ...
        public static readonly DiagnosticDescriptor EventTypeMustBePartial =
            new(
                ...
                "Event type '{0}' must be partial to allow metadata registration",
                Category,
                DiagnosticSeverity.Error,
                true);
