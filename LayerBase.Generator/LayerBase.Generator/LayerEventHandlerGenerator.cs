...
public sealed class LayerEventHandlerGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(compilationAndRegistrations, static (spc, source) =>
        {
            ...
            if (layerSymbol == null || (eventHandlerSymbol == null && eventHandlerAsyncSymbol == null)) return;

            var validBindings = new List<HandlerBinding>();
            foreach (var registration in collected)
            {
                ...
                var implementations =
                    GetEventHandlerInterfaces(handlerType, eventHandlerSymbol, eventHandlerAsyncSymbol).ToList();
                if (implementations.Count == 0) continue;

                foreach (var impl in implementations)
                    validBindings.Add(new HandlerBinding(handlerType, targetLayer, impl.EventType, impl.Kind,
                        location));
            }
    ...
    private static IEnumerable<EventHandlerImplementation> GetEventHandlerInterfaces(
        INamedTypeSymbol handlerType, INamedTypeSymbol? syncInterface, INamedTypeSymbol? asyncInterface)
    {
        if (syncInterface == null && asyncInterface == null) yield break;
        ...
        foreach (var iface in handlerType.AllInterfaces.OfType<INamedTypeSymbol>())
        {
            ...
            else if (asyncInterface != null &&
                     SymbolEqualityComparer.Default.Equals(iface.OriginalDefinition, asyncInterface))
    ...
    private static string GenerateLayerPartial(INamedTypeSymbol layerType, IEnumerable<HandlerBinding> bindings)
    {
        ...
        {
            if (binding.EventType == null) continue;
...
#pragma warning disable RS2008 // Enable analyzer release tracking
    private static class Diagnostics
    {
        ...
        public static readonly DiagnosticDescriptor HandlerNeedsPublicParameterlessConstructor =
            new(
                "LBG104",
                "Event handler needs parameterless constructor",
                "Event handler '{0}' must have a public or internal parameterless constructor",
                Category,
                DiagnosticSeverity.Error,
                true);
        ...
        public static readonly DiagnosticDescriptor HandlerCannotBeAbstract =
            new(
                "LBG105",
                "Event handler cannot be abstract",
                "Event handler '{0}' cannot be abstract when used with OwnerLayerAttribute",
                Category,
                DiagnosticSeverity.Error,
                true);
    }
#pragma warning restore RS2008
    ...
    {
        public HandlerBinding(INamedTypeSymbol handlerType, INamedTypeSymbol layerType, ITypeSymbol? eventType,
                              EventHandlerKind kind,        Location?        location)
        {
            HandlerType = handlerType;
            LayerType = layerType;
            EventType = eventType;
    ...
    {
        public static readonly HandlerBindingComparer Instance = new();
        ...
        public int GetHashCode(HandlerBinding obj)
        {
            ...
            hash = (hash * 397) ^
                   (obj.EventType != null ? SymbolEqualityComparer.Default.GetHashCode(obj.EventType) : 0);
