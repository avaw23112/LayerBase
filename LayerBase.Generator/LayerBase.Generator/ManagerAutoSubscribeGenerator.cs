...
public sealed class ManagerAutoSubscribeGenerator : IIncrementalGenerator
{
    ...
    private static ClassMeta? GetClassMeta(GeneratorSyntaxContext ctx)
    {
        ...
        if (handlers.Count > 0 || delayProps.Count > 0 || implementsCtx)
            return new ClassMeta(symbol.Name, symbol.ContainingNamespace.ToDisplayString(), symbol.ToDisplayString(),
                implementsCtx, handlers, delayProps);
        return null;
    }
    ...
    private static void GenerateCode(SourceProductionContext spc, ClassMeta meta)
    {
        ...
        {
            // 目前只针对同步阻塞方法进行极致优化
            if (h.Attr.Contains("Async") || h.Attr.Contains("Parallel")) continue;

            sb.AppendLine("        [MethodImpl(MethodImplOptions.AggressiveInlining)]");
            sb.AppendLine($"        private static unsafe global::LayerBase.Core.Event.EventHandledState __Bridge_{h.Name}(object instance, in {h.Evt} e)");
            sb.AppendLine("        {");
            sb.AppendLine($"            return (({meta.ClassName})instance).{h.Name}(in e);");
            sb.AppendLine("        }");
        }
        ...
        if (meta.Handlers.Count > 0 || meta.DelayProps.Count > 0)
        {
            ...
            foreach (var h in meta.Handlers)
            {
                if (h.Attr.Contains("Async"))
                {
                    sb.AppendLine($"            layer.SubscribeAsync<{h.Evt}>(this.{h.Name});");
                }
                else if (h.Attr.Contains("Parallel"))
                {
                    sb.AppendLine($"            layer.SubscribeParallel<{h.Evt}>(this.{h.Name});");
                }
                else
                {
                    // 🚀 透明优化路径：通过函数指针订阅
                    sb.AppendLine("            unsafe {");
                    sb.AppendLine($"                delegate*<object, in {h.Evt}, global::LayerBase.Core.Event.EventHandledState> ptr = &__Bridge_{h.Name};");
                    sb.AppendLine($"                layer.SubscribeOptimized<{h.Evt}>((IntPtr)ptr, this, \"{meta.ClassName}.{h.Name}\");");
                    sb.AppendLine("            }");
                }
            }
