...
public sealed class AssemblyModuleGenerator : IIncrementalGenerator
{
        ...
        ImmutableArray<ResourceContribution?> nullableResourceImports)
    {
        ...
        if (modules.Length > 1)
        {
            string moduleNames = string.Join(", ", modules.Select(static item => item.FullTypeName));
            for (int i = 1; i < modules.Length; i++)
            {
                context.ReportDiagnostic(Diagnostic.Create(
                    Diagnostics.MultipleAssemblyModules,
                    modules[i].Location,
                    moduleNames));
            }
        ...
        var resourceExports = nullableResourceExports
                              ...
                              .ThenBy(static item => item.MemberName, StringComparer.Ordinal)
                              .ToImmutableArray();
        ...
        foreach (ModuleInfo module in modules)
        {
            ...
            GenerateModulePartial(
                context,
                module,
                handlers.Any(static handler => handler.Kind == "Call"),
                handlers.Any(static handler => handler.Kind == "Event"));
            GenerateManifest(
                context,
                module,
                layerContracts,
    ...
    private static void GenerateModulePartial(
        SourceProductionContext context,
        ModuleInfo module,
        bool hasCallHandlers,
        bool hasEventHandlers)
    {
        ...
        string dispatchInterface = hasCallHandlers || hasEventHandlers
            ? ", global::LayerBase.Modules.IModuleScopeDispatchProvider"
            : "";
        builder.AppendLine($"    {module.Accessibility} partial class {module.Name} : global::LayerBase.Modules.ILayerBaseModule{dispatchInterface}");
        builder.AppendLine("    {");
        ...
        if (hasCallHandlers || hasEventHandlers)
        {
            builder.AppendLine();
            builder.Append("        global::LayerBase.Scope.ModuleCallDispatchHandler? global::LayerBase.Modules.IModuleScopeDispatchProvider.ModuleCallDispatcher => ");
            builder.AppendLine(hasCallHandlers
                ? "global::LayerBase.Scope.GeneratedScopeCallDispatcher.DispatchModule;"
                : "null;");
            builder.AppendLine();
            builder.Append("        global::LayerBase.Scope.ModuleEventDispatchHandler? global::LayerBase.Modules.IModuleScopeDispatchProvider.ModuleEventDispatcher => ");
            builder.AppendLine(hasEventHandlers
                ? "global::LayerBase.Scope.GeneratedScopePostDispatcher.DispatchModule;"
                : "null;");
        }
        builder.AppendLine("    }");
    ...
    private static void AppendHandlers(StringBuilder builder, ImmutableArray<HandlerContribution> handlers)
    {
        ...
        for (int i = 0; i < handlers.Length; i++)
        {
            ...
            int localHandlerId = handler.Kind == "Call"
                ? callHandlerId++
                : eventHandlerId++;
            ...
            builder.AppendLine($"                    {localHandlerId},");
    ...
    private static void AppendResourceExports(StringBuilder builder, ImmutableArray<ResourceContribution> exports)
    {
        ...
        {
            int localSlot = 0;
            foreach (ResourceContribution export in group
                         .OrderBy(static item => item.SourceStart)
                         .ThenBy(static item => item.MemberName, StringComparer.Ordinal))
            {
    ...
    private static void AppendResourceImports(StringBuilder builder, ImmutableArray<ResourceContribution> imports)
    {
        ...
        {
            int localSlot = 0;
            foreach (ResourceContribution import in group
                         .OrderBy(static item => item.SourceStart)
                         .ThenBy(static item => item.MemberName, StringComparer.Ordinal))
            {
    ...
    private static class Diagnostics
    {
        ...
        public static readonly DiagnosticDescriptor MultipleAssemblyModules = new(
            "LBM002",
            "Only one [AssemblyModule] is allowed per assembly",
            "An assembly can declare only one [AssemblyModule] until module ownership metadata is explicit. Found: {0}",
            "LayerBase.Modules",
            DiagnosticSeverity.Error,
            isEnabledByDefault: true);
    }
