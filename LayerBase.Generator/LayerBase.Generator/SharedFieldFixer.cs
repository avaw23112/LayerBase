...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(SharedFieldFixer)), Shared]
public class SharedFieldFixer : CodeFixProvider
{
    ...
    public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        context.RegisterCodeFix(
            CodeAction.Create(
                title: "Sync type with [Public] source",
                createChangedDocument: c => FixFieldTypeAsync(context.Document, fieldDeclaration, diagnostic, c),
                equivalenceKey: nameof(SharedFieldFixer)),
            diagnostic);
    }

    private async Task<Document> FixFieldTypeAsync(Document document, FieldDeclarationSyntax field, Diagnostic diagnostic, CancellationToken cancellationToken)
    {
